...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	calories := (walkingCaloriesWeightMultiplier*weight + (math.Pow((meanSpeed(action, duration)*kmhInMsec), 2)/(height/float64(cmInM)))*walkingSpeedHeightMultiplier*weight) * duration * float64(minInH)
	return calories
}
